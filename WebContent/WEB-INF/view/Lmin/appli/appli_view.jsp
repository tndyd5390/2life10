...
<%
	AppliDTO aDTO = (AppliDTO)request.getAttribute("aDTO");
...
<form action="#" name="menuFrm" method="post">
	<input type="hidden" name="menuNum"/>
			...
			<div class="contents"> <!-- 페이지별 ID none -->
				<h3 class="smallTit">가입 신청</h3>
				<br/><br/>
				<!-- boardView -->
				<form id="f" name="f" method="post" action="/Lmin/appli/appliReplyProc.do">
				<input type="hidden" id="appliNo" name="appliNo" value="<%=CmmUtil.nvl(aDTO.getAppliNo())%>">
				<article class="boardView">
					<header class="boardTit">
						<h3><%=AES256Util.strDecode(CmmUtil.nvl(aDTO.getAppliName()))%></h3>
						<p class="txt">
							<span class="date"><%=CmmUtil.nvl(aDTO.getAppliProdCodeName())%></span>
							<span class="writer"><%=CmmUtil.nvl(aDTO.getAppliContractCodeName())%></span>
							<span class="writer"><%=CmmUtil.nvl(aDTO.getRegDt()) %></span>
						</p>
					</header>
