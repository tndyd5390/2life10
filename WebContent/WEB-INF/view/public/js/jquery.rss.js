(function ($) {
  ...
  RSS.prototype.getTokenMap = function (entry) {
    ...
    return $.extend({
      ...
      url:       entry.link,
      author:    entry.author,
      date:      this.getFormattedDate(entry.publishedDate).substring(0,10).replace("-", ".").replace("-", "."),
      title:     entry.title.substring(0, 28)+"...",
      //title:     entry.title,
      body:      entry.content,
      ...
      shortBodyPlain: entry.contentSnippet.replace(/<\/?[^>]+>/gi, '').replace("&nbsp;", " ").replace("&nbsp;", " ").replace("&nbsp;", " ").substring(0, 35)+"...",
      ...
      teaserImageUrl1: (function (entry) {
          try {
            return entry.content.match(/(<img.*?>)/gi)[1].match(/src="(.*?)"/)[1];
          }
          catch (e) {
            return '';
          }
